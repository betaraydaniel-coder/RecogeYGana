...
- Unity import, test, build, and Play Mode checks require an activated Unity license. Without one, batch mode exits with `No valid Unity Editor license found`; use the repo's README GitHub Actions licensing notes or a manual Unity `.ulf` license before rerunning those checks.
